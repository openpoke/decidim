...
# two_factor: :verified skips the challenge and two_factor: :bypassed trusts the provider.
Warden::Manager.after_set_user except: :fetch do |user, warden, options|
  ...
  case options[:two_factor]
  when :verified then next
  when :bypassed then session["decidim_two_factor_bypassed"] = user.id
  else
    next unless user.two_factor_challenge_required?
